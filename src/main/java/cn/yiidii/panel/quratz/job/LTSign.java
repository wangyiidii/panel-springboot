package cn.yiidii.panel.quratz.job;

import cn.yiidii.panel.common.utils.dto.HttpClientResult;
import cn.yiidii.panel.common.utils.HttpClientUtil;
...
public class LTSign implements Job {
    ...
    private void sign(String phone, String cookie) throws Exception {
        ...
        if (result.getCode() == 200) {
            ...
            if (StringUtils.equals("0000", msgCode)) {
                log.debug("签到成功！获得了 " + jo.getString("continuCount") + " 个金币");
            } else if (StringUtils.equals("0008", msgCode)) {
                log.debug("签到失败,今天已签到!");
            } else {
                log.debug("签到失败,cookie过期。");

            }
        } else {
            log.debug("签到失败。[" + result.getCode() + "]: " + result.getContent());
        }
    }
    ...
    private void getCoin() throws Exception {
        ...
        if (result.getCode() == 200) {
            log.debug("当前金币个数：" + result.getContent());
        } else {
